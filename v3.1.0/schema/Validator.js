...
export class Validator {
     ...
     */
    static validateStructure(properties, parentKey) {
    ...
    static validateProperty(prop, key) {
        if ('union' in prop) {
            if (!Array.isArray(prop.union) || prop.union.length === 0)
                throw new SchemaError(`Property '${key}' union must be a non-empty array`);
    ...
    static validateDefaultValue(prop, key) {
        try {
            this.validateValue(prop.default, prop, `${key}(default)`);
        }
        catch (error) {
            throw new SchemaError(`Invalid default value for property '${key}': ${error.message}`);
        }
    }
    /**
    ...
    static validateValue(value, prop, key) {
        if (value === undefined || value === null) {
            if (value === null && prop.nullable)
                return;
            if (value === undefined && !('required' in prop && prop.required))
                return;
        ...
        if ('union' in prop) {
            ...
            const isValid = prop.union.some((subProp) => {
                try {
                    this.validateValue(value, subProp, key);
                    return true;
                }
                catch (e) {
