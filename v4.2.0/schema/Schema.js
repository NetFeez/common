...
export class Schema {
        ...
        if (prop.type === 'union') {
            ...
            for (const subKey in prop.properties) {
                ...
                try {
                    ...
                    if ('required' in subProp && subProp.required)
                        return undefined;
                    continue;
                }
                if (value !== undefined)
    ...
    validatePropertyMath(data, prop, checkDefaults = true) {
        if (data === null)
            return !!prop.nullable;
        if (prop.type === 'union')
            return prop.union.some(subProp => this.validatePropertyMath(data, subProp, checkDefaults));
        ...
        if (prop.type === 'object' && prop.properties) {
            ...
            for (const key in prop.properties) {
                ...
                if (!hasKey) {
                    if (checkDefaults && 'default' in subProp)
                        continue;
                    if (subProp.required)
                        return false;
                }
                else {
                    if (!this.validatePropertyMath(data[key], subProp, checkDefaults))
                        return false;
                }
            }
            return true;
        ...
        if (prop.type === 'array' && Array.isArray(data))
            return data.every(item => this.validatePropertyMath(item, prop.items, checkDefaults));
        return true;
    }
    /**
     * process a property
    ...
    processProperty(data, prop, key, partial = false) {
        ...
        if (prop.type === 'union') {
            ...
            for (const subProp of fullMatches.length ? fullMatches : matches) {
                try {
                    return this.processProperty(data, subProp, key, partial);
                }
                catch {
                    continue;
                }
    ...
    processObject(value, prop, key, partial = false) {
        ...
        for (const valueKey of valueKeys) {
            ...
            if (prop.allowAdditionalProperties === false)
                throw new SchemaError(`Unknown property "${valueKey}" at ${key}`);
